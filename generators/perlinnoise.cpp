   ...
   for (int y = 0; y < size.height(); y++) {
      for (int x = 0; x < size.width(); x++) {
         ...
         for (int currOctave = 0; currOctave < numOctaves - 1; currOctave++) {
            double frequency =
                pow(2, currOctave);  // This increases the frequency with every loop of the octave.
            double amplitude =
                pow(persistence,
                    currOctave);  // This decreases the amplitude with every loop of the octave.
            getnoise += noise(randomizer + static_cast<double>(x) * xFactor * frequency / zoom,
                              randomizer + static_cast<double>(y) * yFactor / zoom * frequency) *
                        amplitude;
         }
         ...
         destimage[thisPos].r =
             qMin(qMax((int)(fraction * (float)color.red() +
                             (blend ? (negVal * (float)sourceImg[thisPos].r) : 0)),
                       0),
                  255);
         destimage[thisPos].g =
             qMin(qMax((int)(fraction * (float)color.green() +
                             (blend ? (negVal * (float)sourceImg[thisPos].g) : 0)),
                       0),
                  255);
         destimage[thisPos].b =
             qMin(qMax((int)(fraction * (float)color.blue() +
                             (blend ? (negVal * (float)sourceImg[thisPos].b) : 0)),
                       0),
                  255);
